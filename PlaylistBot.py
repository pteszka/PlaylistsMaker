    ...
    def get_ids(self):
        ...
        for count, row in enumerate(tbody.find_elements_by_xpath('./tr')):
            if (count + 1) % 8 == 0:
                continue
            try:
                var = self.driver.find_element_by_xpath(tbody + '/tr[{}]/td[3]/div[2]/div'.format(count + 1))
            except:
                continue
        ...
        for counter, album in enumerate(idAlbums):
            if topNumber == 10:
                break
            album_tracks = sp.album_tracks(idAlbums[counter])
